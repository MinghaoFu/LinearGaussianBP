...
from utils import random_zero_array
...
def generate_mixing_matrix(d_sources: int, d_data=None, lin_type='uniform', cond_threshold=25, effect_threshold=3, n_iter_4_cond=None,
...
def generate_data(args, n_per_seg, n_seg, d_sources, d_data=None, n_layers=3, prior='gauss', activation='lrelu', batch_size=0,
    ...
    if not repeat_linearity:
        C_mask = args.synthetic_scale * np.repeat(np.expand_dims(np.cos(np.arange(S.shape[0]) / 100), axis=1), d_sources*d_data, axis=1).reshape(-1, d_data, d_sources) #np.random.uniform(-1.5, 1.5, size=(S.shape[0], d_data, d_sources)) + 
        C_mask[np.abs(C_mask) < args.synthetic_C_thres * args.synthetic_scale] = 0
        C_mask[np.abs(C_mask) >= args.synthetic_C_thres * args.synthetic_scale] = 1
