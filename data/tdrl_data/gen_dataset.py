...
root_dir = '/home/minghao.fu/workspace/LatentTimeVaryingCausalDiscovery/data/'
...
def pnl_modular_gaussian_ts(NClass=1):
    lags = 2
    Nlayer = 3
    length = 1
    condList = []
...
def case2_nonstationary_causal():
    ...
    mixingList.append(np.random.randn(latent_size, observed_size))
    ...
    xt_ns = np.vstack(xt_ns)
    ct_ns = np.vstack(ct_ns)
    print(yt_ns.shape, xt_ns.shape, ct_ns.shape)
    np.savez(os.path.join(path, "data"), 
            yt = yt_ns, 
            xt = xt_ns,
            ct = ct_ns)
...
if __name__ == "__main__":
    ...
    #pnl_modular_gaussian_ts(NClass=1)